...
main = do
  ...
  hakyll $ do
    ...
    create ["css/style.css"] $ do
      route idRoute
      compile $ makeItem $ T.unpack $ V.style

    create ["css/code.css"] $ do
      route idRoute
      compile $ makeItem $ T.unpack $ V.code

    create ["index.html"] $ do
      route idRoute
      compile $ makeItem $ T.unpack $ V.index (mostRecent 5 posts)

    create ["feed.xml"] $ do
      route idRoute
      compile $ makeItem $ T.unpack $ V.feed $ sorted posts

    create ["posts/index.html"] $ do
      route idRoute
      compile $ makeItem $ T.unpack $ V.archive $ sorted posts

    create ["publications/index.html"] $ do
      route idRoute
      compile $ makeItem $ T.unpack $ V.publications

    forM_ posts $ \post -> do
      create [fromFilePath $ "posts" </> ST.unpack (slug post) </> "index.html"] $ do
        route idRoute
        compile $ makeItem $ T.unpack $ V.post post
